...
void main() {
    ...
    v_shadowMapUv.z = min(v_shadowMapUv.z, 0.996);
    #endif//shadowMapFlag
    ...
    pointLightColor = v_lightDiffuse;
    for (int i = 0; i < numPointLights; i++) {
        ...
        //if(vecToFloat(v_lightDiffuse) < vecToFloat(u_pointLights[i].color* (NdotL / (1.0 + dist2))))
        if (vecToFloat(pointLightColor) <  vecToFloat(value))
            pointLightColor = mix(pointLightColor, value, 1.0);
        #ifdef specularFlag
